/*
Copyright (C) 2023-2026 QuantumNous

This program is free software: you can redistribute it and/or modify
it under the terms of the GNU Affero General Public License as
published by the Free Software Foundation, either version 3 of the
License, or (at your option) any later version.

This program is distributed in the hope that it will be useful,
but WITHOUT ANY WARRANTY; without even the implied warranty of
MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
GNU Affero General Public License for more details.

You should have received a copy of the GNU Affero General Public License
along with this program. If not, see <https://www.gnu.org/licenses/>.

For commercial licensing, please contact support@quantumnous.com
*/
import {
  Activity,
  BadgeDollarSign,
  Braces,
  GitBranch,
  KeyRound,
  Layers3,
  ShieldCheck,
  Video,
} from 'lucide-react'
import { useTranslation } from 'react-i18next'
import { AnimateInView } from '@/components/animate-in-view'

interface FeaturesProps {
  className?: string
}

export function Features(_props: FeaturesProps) {
  const { t } = useTranslation()

  const features = [
    {
      id: 'models',
      title: t('Model marketplace'),
      desc: t(
        'Browse text, image, audio and video models in one place and expose them through a consistent API surface.'
      ),
      icon: <Layers3 className='size-5' strokeWidth={1.5} />,
      span: 'md:col-span-2',
      visual: ['GPT-4o', 'Claude', 'Gemini', 'DeepSeek', 'Seedance'],
    },
    {
      id: 'routing',
      title: t('Reliable routing'),
      desc: t(
        'Route by model, group, priority and health so production traffic keeps flowing when an upstream changes.'
      ),
      icon: <GitBranch className='size-5' strokeWidth={1.5} />,
      span: 'md:col-span-1',
      visual: [t('fallback'), t('priority'), t('retry')],
    },
    {
      id: 'billing',
      title: t('Usage based billing'),
      desc: t(
        'Set model prices, group discounts and pre-consumption rules before requests reach the upstream provider.'
      ),
      icon: <BadgeDollarSign className='size-5' strokeWidth={1.5} />,
      span: 'md:col-span-1',
      visual: ['$0.002', '$0.009', '$0.958'],
    },
    {
      id: 'video',
      title: t('Video generation ready'),
      desc: t(
        'Expose asynchronous generation tasks such as Seedance through the same authenticated gateway.'
      ),
      icon: <Video className='size-5' strokeWidth={1.5} />,
      span: 'md:col-span-2',
      visual: ['/v1/video/generations', '/v1/tasks/{id}'],
    },
  ]

  const compactFeatures = [
    {
      icon: <KeyRound className='size-5' strokeWidth={1.5} />,
      title: t('One user key'),
      desc: t('Users call Deeprouter keys while admins manage upstream keys.'),
    },
    {
      icon: <Activity className='size-5' strokeWidth={1.5} />,
      title: t('Live observability'),
      desc: t('Track requests, spend, latency and channel failures.'),
    },
    {
      icon: <ShieldCheck className='size-5' strokeWidth={1.5} />,
      title: t('Quota controls'),
      desc: t('Limit usage by user, key, group, model and IP.'),
    },
    {
      icon: <Braces className='size-5' strokeWidth={1.5} />,
      title: t('OpenAI compatible'),
      desc: t('Keep familiar request shapes and switch models fast.'),
    },
  ]

  return (
    <section className='bg-background text-foreground dark:bg-[#05060a] relative z-10 px-6 py-24 md:py-32 dark:text-white'>
      <div className='mx-auto max-w-6xl'>
        <AnimateInView className='mb-16 max-w-2xl'>
          <p className='mb-3 text-xs font-medium tracking-widest text-blue-600 uppercase dark:text-blue-300'>
            {t('Why Deeprouter')}
          </p>
          <h2 className='text-3xl leading-tight font-semibold tracking-[-0.04em] md:text-5xl'>
            {t('The routing layer between your app and every model.')}
          </h2>
        </AnimateInView>

        <div className='grid gap-4 md:grid-cols-3'>
          {features.map((feature, i) => (
            <AnimateInView
              key={feature.id}
              delay={i * 100}
              animation='scale-in'
              className={`border-border bg-card/70 hover:bg-muted/60 group rounded-3xl border p-6 transition-colors dark:border-white/10 dark:bg-white/[0.04] dark:hover:bg-white/[0.07] ${feature.span}`}
            >
              <div className='mb-5 flex items-center gap-3'>
                <div className='flex size-10 items-center justify-center rounded-2xl bg-blue-500/10 text-blue-600 dark:bg-white/10 dark:text-blue-200'>
                  {feature.icon}
                </div>
                <h3 className='text-base font-semibold'>{feature.title}</h3>
              </div>
              <p className='text-muted-foreground max-w-xl text-sm leading-relaxed dark:text-slate-400'>
                {feature.desc}
              </p>
              <div className='mt-6 flex flex-wrap gap-2'>
                {feature.visual.map((item) => (
                  <span
                    key={item}
                    className='border-border bg-muted/50 text-muted-foreground rounded-full border px-3 py-1 text-xs dark:border-white/10 dark:bg-black/20 dark:text-slate-300'
                  >
                    {item}
                  </span>
                ))}
              </div>
            </AnimateInView>
          ))}
        </div>

        <div className='mt-12 grid gap-4 md:grid-cols-4'>
          {compactFeatures.map((feature, i) => (
            <AnimateInView
              key={feature.title}
              delay={i * 80}
              animation='fade-up'
              className='border-border bg-card/60 rounded-2xl border p-5 dark:border-white/10 dark:bg-white/[0.025]'
            >
              <div className='text-muted-foreground mb-4 dark:text-slate-300'>
                {feature.icon}
              </div>
              <h3 className='mb-2 text-sm font-semibold'>{feature.title}</h3>
              <p className='text-muted-foreground text-xs leading-relaxed dark:text-slate-500'>
                {feature.desc}
              </p>
            </AnimateInView>
          ))}
        </div>
      </div>
    </section>
  )
}
